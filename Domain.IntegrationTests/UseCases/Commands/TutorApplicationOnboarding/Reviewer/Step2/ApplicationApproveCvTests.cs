...
public class ApplicationApproveCvTests : BaseTest
{
    [Fact]
    public async Task Handle_ApprovesPendingTutorApplicationAndSendsEmailsToApplicantAndReviewer()
    {
        ...
        var command = new ApplicationApproveCv.Command(tutorApplication.Id);
        ...
        Assert.Equal(TutorApplication.OnboardingStep.InterviewBooking, updatedTutorApplication.CurrentStep);

        // Assert interview was created
        var interview = updatedTutorApplication.Interview;
        Assert.NotNull(interview);
        Assert.Equal(TutorApplicationInterview.InterviewStatus.AwaitingApplicantSlotSelection, interview.Status);
    ...
    {
        // Arrange
        var command = new ApplicationApproveCv.Command(Guid.NewGuid());
