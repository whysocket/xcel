...
internal static class ApplicationApproveCvCommandErrors
{
    ...
    public static Error EmailSendFailed(Guid id) =>
        new(
            ...
            $"Failed to send approval emails for Tutor Application ID: '{id}'"
        );
}
...
) : IApplicationApproveCvCommand
{
    ...
    )
    {
        logger.LogInformation(
            "{Service} Approving CV for TutorApplicationId: {Id}",
            ServiceName,
            tutorApplicationId
        );

        var tutorApplication =
            await tutorApplicationsRepository.GetByIdWithDocumentsAndApplicantAsync(
                tutorApplicationId,
                cancellationToken
            );
        if (tutorApplication is null)
        {
            ...
            return Result.Fail(
                ApplicationApproveCvCommandErrors.TutorApplicationNotFound(tutorApplicationId)
            );
        }
        ...
        var reviewerResult = await reviewerAssignmentService.GetAvailableReviewerAsync(
            cancellationToken
        );
        if (reviewerResult.IsFailure)
        {
            logger.LogError(
                "{Service} No reviewer available for TutorApplicationId: {Id}",
                ServiceName,
                tutorApplicationId
            );
        ...
        var cvDocument = tutorApplication.Documents.Single(d =>
        ...
        logger.LogInformation(
            "{Service} Assigned reviewer and updated interview for TutorApplicationId: {Id}",
            ServiceName,
            tutorApplicationId
        );
        ...
        {
            logger.LogError(
                "{Service} Email send failed for TutorApplicationId: {Id}",
                ServiceName,
                tutorApplicationId
            );
            return Result.Fail(
                ApplicationApproveCvCommandErrors.EmailSendFailed(tutorApplicationId)
            );
        }

        logger.LogInformation(
            "{Service} Emails sent to Applicant: {ApplicantEmail} and Reviewer: {ReviewerEmail}",
            ServiceName,
            tutorApplication.Applicant.EmailAddress,
            reviewer.EmailAddress
        );
