...
public static class ApplicantProposeInterviewDates
{
    ...
    ) : IRequestHandler<Command, Result>
    {
        public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            {
                logger.LogWarning(
                    "[ProposeInterviewDates] Application or interview not found for ID: {TutorApplicationId}",
                    tutorApplicationId);
            ...
            {
                logger.LogWarning(
                    "[ProposeInterviewDates] Invalid interview status: {Status} for TutorApplicationId: {TutorApplicationId}",
            ...
            {
                logger.LogError(
                    "[ProposeInterviewDates] Failed to send email to reviewer: {ReviewerEmail}, Errors: {@Errors}",
            ...
            logger.LogInformation(
                "[ProposeInterviewDates] Interview dates updated and status set to AwaitingReviewerConfirmation for {TutorApplicationId}",
                tutorApplicationId);
