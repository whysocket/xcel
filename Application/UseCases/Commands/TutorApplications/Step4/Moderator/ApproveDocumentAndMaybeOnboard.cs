...
public static class ApproveDocumentAndMaybeOnboard
{
    ...
    ) : IRequestHandler<Command, Result>
    {
        public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            logger.LogInformation("[ApproveDocument] Document {Id} fully processed", document.Id);
            return Result.Ok();
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (result.IsFailure)
            {
                logger.LogError("[ApproveDocument] Failed to send approval email: {@Errors}", result.Errors);
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (result.IsFailure)
            {
                logger.LogError("[ApproveDocument] Failed to send onboarding email: {@Errors}", result.Errors);
                return Result.Fail(result.Errors);
            }

            logger.LogInformation("[ApproveDocument] TutorApplication {Id} marked as Onboarded", application.Id);
            return Result.Ok();
        }
    }
}