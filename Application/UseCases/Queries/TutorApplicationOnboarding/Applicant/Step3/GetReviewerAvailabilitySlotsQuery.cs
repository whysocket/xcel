...
internal static class GetReviewerAvailabilitySlotsQueryErrors
{
    public static Error TutorApplicationNotFound =>
        new(ErrorType.Forbidden, $"Tutor application not found");
...
) : IGetReviewerAvailabilitySlotsQuery
{
    ...
    public async Task<Result<List<TimeSlot>>> ExecuteAsync(
        Guid applicantUserId,
        DateOnly dateUtc,
        CancellationToken cancellationToken = default
    )
    {
        logger.LogInformation(
            "{Service} Fetching reviewer availability for applicant {ApplicantId}",
            ServiceName,
            applicantUserId
        );

        var application = await tutorApplicationsRepository.GetByUserIdAsync(
            applicantUserId,
            cancellationToken
        );
        ...
        {
            logger.LogWarning(
                "{Service} Tutor application not found for ID user {applicantUserId}",
                ServiceName,
                applicantUserId
            );
            return Result.Fail<List<TimeSlot>>(
                GetReviewerAvailabilitySlotsQueryErrors.TutorApplicationNotFound
            );
        }
        ...
        if (application.ApplicantId != applicantUserId)
        {
            ...
            return Result.Fail<List<TimeSlot>>(
                GetReviewerAvailabilitySlotsQueryErrors.Unauthorized(
                    applicantUserId,
                    application.Id
                )
            );
        ...
        {
            logger.LogWarning(
                "{Service} No reviewer assigned for application {ApplicationId}",
                ServiceName,
                application.Id
            );
            return Result.Fail<List<TimeSlot>>(
        ...
        logger.LogInformation(
            "{Service} Getting availability for reviewer {ReviewerId} on {Date}",
            ServiceName,
            reviewerId,
            dateUtc
        );
        ...
        {
            logger.LogError(
                "{Service} Failed to fetch availability for reviewer {ReviewerId}. Errors: {@Errors}",
                ServiceName,
                reviewerId,
                result.Errors
            );
