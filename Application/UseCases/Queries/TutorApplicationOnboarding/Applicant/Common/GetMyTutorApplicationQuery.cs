...
internal static class GetMyTutorApplicationQueryErrors
{
    ...
    internal static Error AlreadyHasRole(Guid userId) =>
        new(
            ErrorType.Conflict,
            $"User '{userId}' already has a role and cannot access the onboarding application."
        );
}
...
internal sealed class GetMyTutorApplicationQuery(
    ...
    ILogger<GetMyTutorApplicationQuery> logger
) : IGetMyTutorApplicationQuery
{
    ...
    )
    {
        ...
        {
            logger.LogError(
                "{Service} Failed to fetch roles for user {UserId}: {Errors}",
                ServiceName,
                userId,
        ...
        if (userRolesResult.Value.Any(role => UserRoles.All.Contains(role.Name)))
        {
            ...
            return Result.Fail<TutorApplication>(
                GetMyTutorApplicationQueryErrors.AlreadyHasRole(userId)
            );
        }

        logger.LogInformation(
            "{Service} Retrieving tutor application for user {UserId}",
            ServiceName,
            userId
        );
        ...
        {
            logger.LogWarning(
                "{Service} No application found for user {UserId}",
                ServiceName,
                userId
            );
        ...
        logger.LogInformation(
            "{Service} Application {ApplicationId} found for user {UserId}",
            ServiceName,
            application.Id,
            userId
        );
