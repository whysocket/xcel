...
internal class ReviewerAssignmentService(IAuthServiceSdk authServiceSdk)
    : IReviewerAssignmentService
{
    internal static class Errors
    {
        internal static Error ReviewersUnavailability = new(
            ErrorType.Validation,
            "There is no reviewers"
        );
    }

    public async Task<Result<Person>> GetAvailableReviewerAsync(
        CancellationToken cancellationToken = default
    )
    {
        var reviewerRoleResult = await authServiceSdk.GetRoleByNameAsync(
            UserRoles.Reviewer,
            cancellationToken
        );
