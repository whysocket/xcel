...
internal sealed class GetPersonRolesByRoleIdQuery(
    ...
    ILogger<GetPersonRolesByRoleIdQuery> logger
) : IGetPersonRolesByRoleIdQuery
{
    ...
    )
    {
        if (roleId == Guid.Empty)
        {
            ...
            return Result.Fail<PageResult<PersonRoleEntity>>(
                CommonErrors.InvalidGuid(nameof(roleId))
            );
        }
        ...
        logger.LogInformation(
            $"{ServiceName} - Retrieved person-role assignments by role. roleId: {roleId}, page: {pageRequest.PageNumber}, pageSize: {pageRequest.PageSize}, totalCount: {personsRoles.TotalCount}."
        );
