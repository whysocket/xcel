...
internal static class DeleteRoleByNameServiceErrors
{
    internal static Error RoleNotFound(string roleName) =>
        new(ErrorType.NotFound, $"The role '{roleName}' is not found.");
}

internal sealed class DeleteRoleByNameCommand(
    IRolesRepository rolesRepository,
    ILogger<DeleteRoleByNameCommand> logger
) : IDeleteRoleByNameCommand
{
    ...
    )
    {
        ...
        {
            logger.LogWarning(
                $"{ServiceName} - Not Found: Role '{roleName}' not found for deletion."
            );
        ...
        logger.LogInformation(
            $"{ServiceName} - Role deleted successfully. RoleId: {existingRole.Id}, Name: {existingRole.Name}."
        );
        return Result.Ok();
    }
}
