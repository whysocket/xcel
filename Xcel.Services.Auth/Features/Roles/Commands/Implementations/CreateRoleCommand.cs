...
internal static class CreateRoleServiceErrors
{
    internal static Error RoleNameRequired() =>
        new(ErrorType.Validation, "The role name is required");

    internal static Error RoleAlreadyExists(string roleName) =>
        new(ErrorType.Conflict, $"The role '{roleName}' already exists.");
}

internal sealed class CreateRoleCommand(
...
) : ICreateRoleCommand
{
    ...
    )
    {
        ...
        {
            logger.LogWarning(
                $"{ServiceName} - Conflict: Role with name '{roleName}' already exists (Id: {existingRole.Id})."
            );
        ...
        logger.LogInformation(
            $"{ServiceName} - Role created successfully. RoleId: {newRole.Id}, Name: {newRole.Name}."
        );
