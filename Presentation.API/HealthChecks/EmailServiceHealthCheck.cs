...
public class EmailServiceHealthCheck(HttpClient httpClient, InfraOptions infraOptions)
    : IHealthCheck
{
    ...
    )
    {
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return HealthCheckResult.Healthy(
                    "Email service is reachable and healthy.",
                    data: healthData
                );
            }
            ...
            healthData["statusCode"] = (int)response.StatusCode;
            return HealthCheckResult.Unhealthy(
                $"Email service returned status code {response.StatusCode}.",
                data: healthData
            );
        }
        catch (HttpRequestException httpEx)
        {
            ...
            return HealthCheckResult.Unhealthy(
                $"Email service HTTP request failed: {httpEx.Message}",
                exception: httpEx,
                data: healthData
            );
        }
        catch (Exception ex)
        {
            ...
            return HealthCheckResult.Unhealthy(
                $"Email service health check failed: {ex.Message}",
                exception: ex,
                data: healthData
            );
        }
