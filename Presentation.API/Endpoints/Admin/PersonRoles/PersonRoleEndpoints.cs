...
{
    internal static IEndpointRouteBuilder MapPersonRoleEndpoints(
        this IEndpointRouteBuilder endpoints
    )
    {
        endpoints
            .MapPost(
                Endpoints.Admin.PersonRoles.Create,
                async (
                    Guid personId,
                    Guid roleId,
                ...
                {
                    var result = await authService.AddRoleToPersonAsync(
                        personId,
                        roleId,
                        httpContext.RequestAborted
                    );
                    return result.IsSuccess
                        ? Results.Created(Endpoints.Admin.PersonRoles.GetAll, null)
        ...
        endpoints
            .MapGet(
                Endpoints.Admin.PersonRoles.GetAll,
                async (Guid personId, IAuthServiceSdk authService, HttpContext httpContext) =>
                {
                    var result = await authService.GetRolesByPersonIdAsync(
                        personId,
                        httpContext.RequestAborted
                    );
            ...
            .WithDescription(
                "Retrieves the list of roles assigned to a specific user, identified by their person ID."
            )
            .WithTags(UserRoles.Admin)
            .RequireAuthorization(p => p.RequireRole(UserRoles.Admin));
        ...
        endpoints
            .MapDelete(
                Endpoints.Admin.PersonRoles.Delete,
                async (
                    Guid personId,
                    Guid roleId,
                    IAuthServiceSdk authService,
                ...
                {
                    var result = await authService.RemoveRoleFromPersonAsync(
                        personId,
                        roleId,
                        httpContext.RequestAborted
                    );
