...
internal static class TutorApplicationEndpoints
{
    ...
    internal static IEndpointRouteBuilder MapTutorApplicationEndpoints(
        this IEndpointRouteBuilder endpoints
    )
    {
        endpoints
            .MapPost(
                Endpoints.TutorApplications.BasePath,
                async (
                ...
                {
                    var documentPayload = await DocumentPayload.FromFileAsync(
                        body.Cv,
                        context.RequestAborted
                    );

                    var result = await command.ExecuteAsync(
                        new(body.FirstName, body.LastName, body.EmailAddress, documentPayload),
                        context.RequestAborted
                    );

                    return result.IsSuccess
                        ? Results.Created(
                            $"/tutor-applications/{result.Value}",
        ...
        endpoints
            .MapGet(
                Endpoints.TutorApplications.My,
                async (
        ...
        endpoints
            .MapGet(
                Endpoints.TutorApplications.ReviewerAvailabilityByDate,
                async (
