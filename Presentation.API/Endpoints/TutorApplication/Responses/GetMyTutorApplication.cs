...
public record GetMyTutorApplicationResponse(
    [property: Description("Unique identifier of the tutor application.")] Guid Id,
    [property: Description("Current onboarding step of the tutor application.")] string Status,
...
{
    public static GetMyTutorApplicationResponse FromDomain(
        Domain.Entities.TutorApplication application
    )
    {
        return new GetMyTutorApplicationResponse(
            ...
            application.Interview is null
                ? null
                : TutorApplicationInterviewResponse.FromDomain(application.Interview)
        );
    }
}

public record TutorDocumentResponse(
    [property: Description("Unique identifier of the document.")] Guid Id,
    [property: Description("Type of document (e.g., CV, ID, DBS).")] string Type,
...
public record TutorApplicationInterviewResponse(
    [property: Description("Unique identifier of the interview record.")] Guid Id,
    [property: Description("Scheduled date and time of the interview in UTC.")]
        DateTime? ScheduledAt,
    [property: Description(
        "Platform where the interview will take place (e.g., Zoom, Google Meet)."
    )]
        string Platform,
    [property: Description(
        "Current status of the interview (e.g., AwaitingConfirmation, Confirmed)."
    )]
        string Status,
...
public record ReviewerResponse(
    [property: Description("Unique identifier of the reviewer.")] Guid Id,
    [property: Description("First name of the reviewer.")] string FirstName,
    [property: Description("Last name of the reviewer.")] string LastName
)
{
    public static ReviewerResponse FromDomain(Person reviewer)
    {
        return new ReviewerResponse(reviewer.Id, reviewer.FirstName, reviewer.LastName);
    }
}
