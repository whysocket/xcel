...
)
{
    ...
    private object CreateMetadataEmbed(HttpContext httpContext)
    {
        return new
        {
            ...
            fields = new[]
            {
                ...
                {
                    name = "User Agent",
                    value = httpContext.Request.Headers.UserAgent.ToString(),
                    inline = false,
                },
                new
                {
                    name = "Method",
                    value = httpContext.Request.Method,
                    inline = true,
                },
                new
                {
                ...
                {
                    name = "IP Address",
                    value = httpClientInfoService.IpAddress,
                    inline = true,
                },
            },
        };
    ...
    private object CreateDeveloperEmbed(Exception exception, HttpContext httpContext)
    {
        ...
        var formattedHeaders = headers.Select(header =>
        {
            var key = header.Key;
            var value =
                header.Value.Length > 50 ? header.Value.Substring(0, 50) + "..." : header.Value; // Truncate long values
            return $"{key}: {value}";
        });
