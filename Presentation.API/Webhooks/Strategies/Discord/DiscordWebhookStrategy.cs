    ...
    public async Task SendWebhookAsync(object payload)
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                logger.LogError(
                    $"[DiscordWebhookStrategy] Error sending Discord notification. Status: {response.StatusCode}, Content: {responseContent}"
                );
            }
            else
            {
                logger.LogInformation(
                    "[DiscordWebhookStrategy] Discord notification sent successfully."
                );
            }
        }
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "[DiscordWebhookStrategy] Error sending Discord notification: {Message}",
                ex.Message
            );
        }
    }
